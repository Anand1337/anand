...
use crate::peer_manager::peer::peer_actor::PeerActor;
use crate::peer_manager::peer_manager::peer_store::{PeerStore, TrustLevel};
use crate::peer_manager::private_actix::{
    PeerRequestResult, PeersRequest, RegisterPeer, RegisterPeerResponse, SendMessage, StopMsg,
    Unregister, 
};
use crate::peer_manager::routing::edge_validator_actor::EdgeValidatorHelper;
use crate::peer_manager::routing::routing_table_actor::{
...
use near_network_primitives::types::{
    ...
    KnownProducer, NetworkConfig, NetworkViewClientMessages, 
...
impl PeerManagerActor {
                ...
                .then(move |response| match response.map(|r| r.into_inner()) {
                    Ok(RoutingTableMessagesResponse::StartRoutingTableSyncResponse(response)) => {
                        addr.do_send(SendMessage {
                            message: crate::peer_manager::types::PeerMessage::RoutingTableSyncV2(response),
                        });
    ...
    fn handle_msg_start_routing_table_sync(
        &self,
        msg: crate::peer_manager::private_actix::StartRoutingTableSync,
        ctx: &mut Context<Self>,
        throttle_controller: Option<ThrottleController>,
    ) {
    ...
    #[cfg(feature = "test_features")]
    #[perf]
    fn handle_msg_set_adv_options(&mut self, msg: crate::peer_manager::test_utils::SetAdvOptions) {
    ...
    fn handle_msg_set_routing_table(
        &mut self,
        msg: crate::peer_manager::test_utils::SetRoutingTable,
        ctx: &mut Context<Self>,
    ) {
    ...
    fn handle_msg_get_peer_id(
        &self,
        _msg: crate::peer_manager::private_actix::GetPeerId,
    ) -> crate::peer_manager::private_actix::GetPeerIdResult {
        crate::peer_manager::private_actix::GetPeerIdResult { peer_id: self.my_peer_id.clone() }
    }
    ...
    fn process_ibf_msg(
        &self,
        peer_id: &PeerId,
        mut ibf_msg: crate::peer_manager::network_protocol::RoutingVersion2,
        addr: Addr<PeerActor>,
        throttle_controller: Option<ThrottleController>,
    ) {
                            ...
                            if let Some(response_ibf_msg) = response_ibf_msg {
                                addr.do_send(SendMessage {
                                    message: PeerMessage::RoutingTableSyncV2(
                                        crate::peer_manager::network_protocol::RoutingSyncV2::Version2(
                                            response_ibf_msg,
                                        ),
                                    ),
                                });
