...
use crate::peer_manager::peer::tracker::Tracker;
use crate::peer_manager::peer::utils;
use crate::peer_manager::private_actix::{
    PeersRequest, RegisterPeer, RegisterPeerResponse, SendMessage, Unregister,
};
use crate::peer_manager::stats::metrics::{self, NetworkMetrics};
        ...
        match (self.peer_status, peer_msg) {
            (_, PeerMessage::HandshakeFailure(peer_info, reason)) => {
                match reason {
                    ...
                    HandshakeFailureReason::InvalidTarget => {
                        ...
                        self.peer_manager_addr.do_send(
                            PeerManagerMessageRequest::PeerRequest(PeerRequest::UpdatePeerInfo(peer_info)),
                        );
                    }
                }
                ctx.stop();
            ...
            (PeerStatus::Connecting, PeerMessage::Handshake(handshake)) => {
                ...
                self.peer_manager_addr
                    .send(PeerManagerMessageRequest::RegisterPeer(RegisterPeer {
                        actor: ctx.address(),
