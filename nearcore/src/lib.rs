...
pub fn start_with_config_and_synchronization(
    ...
    /*if 2 + 2 == 4 {
        ...
        let mut i: u64 = 0;
        for (key, value) in store.iter(DBCol::State) {
            i += 1;
            let batch_size = 1000;
            if i % batch_size == 0 {
                println!("Processed {} keys in column {} ({})", i, DBCol::State, DBCol::State as usize);
                store_update.commit().unwrap();
                store_update = store.store_update();
            }
            store_update.set(DBCol::State128MIB, &key, &value);
            store_update.set(DBCol::State256MIB, &key, &value);
            store_update.set(DBCol::State512MIB, &key, &value);
        }
