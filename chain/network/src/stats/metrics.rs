...
use near_metrics::near_gauge::{NearHistogram, NearIntCounter, NearIntGauge};

use near_metrics::{inc_counter_by_opt, inc_counter_opt, try_create_int_counter, IntCounter};

use crate::types::{PeerMessage, RoutedMessageBody};

pub static PEER_CONNECTIONS_TOTAL: Lazy<NearIntGauge> =
    Lazy::new(|| NearIntGauge::new("near_peer_connections_total", "Number of connected peers"));

pub static PEER_DATA_RECEIVED_BYTES: Lazy<NearIntCounter> = Lazy::new(|| {
    NearIntCounter::new("near_peer_data_received_bytes", "Total data received from peers")
});
pub static PEER_MESSAGE_RECEIVED_TOTAL: Lazy<NearIntCounter> = Lazy::new(|| {
    NearIntCounter::new(
        "near_peer_message_received_total",
        "Number of messages received from peers",
    )
});
pub static PEER_CLIENT_MESSAGE_RECEIVED_TOTAL: Lazy<NearIntCounter> = Lazy::new(|| {
    NearIntCounter::new(
        "near_peer_client_message_received_total",
        "Number of messages for client received from peers",
    )
});
pub static PEER_BLOCK_RECEIVED_TOTAL: Lazy<NearIntCounter> = Lazy::new(|| {
    NearIntCounter::new("near_peer_block_received_total", "Number of blocks received by peers")
});
pub static PEER_TRANSACTION_RECEIVED_TOTAL: Lazy<NearIntCounter> = Lazy::new(|| {
    NearIntCounter::new(
...
pub static ROUTING_TABLE_RECALCULATIONS: Lazy<NearIntCounter> = Lazy::new(|| {
    NearIntCounter::new(
...
pub static ROUTING_TABLE_RECALCULATION_HISTOGRAM: Lazy<NearHistogram> = Lazy::new(|| {
    NearHistogram::new(
...
pub static EDGE_UPDATES: Lazy<NearIntCounter> =
    Lazy::new(|| NearIntCounter::new("near_edge_updates", "Unique edge updates"));
pub static EDGE_ACTIVE: Lazy<NearIntGauge> =
    Lazy::new(|| NearIntGauge::new("near_edge_active", "Total edges active between peers"));
pub static PEER_REACHABLE: Lazy<NearIntGauge> = Lazy::new(|| {
    NearIntGauge::new(
        "near_peer_reachable",
        "Total peers such that there is a path potentially through other peers",
    )
});
pub static DROP_MESSAGE_UNKNOWN_ACCOUNT: Lazy<NearIntCounter> = Lazy::new(|| {
    NearIntCounter::new(
        "near_drop_message_unknown_account",
        "Total messages dropped because target account is not known",
    )
});
pub static RECEIVED_INFO_ABOUT_ITSELF: Lazy<NearIntCounter> = Lazy::new(|| {
    NearIntCounter::new(
        "received_info_about_itself",
        "Number of times a peer tried to connect to itself",
    )
});
pub static DROPPED_MESSAGES_COUNT: Lazy<NearIntCounter> = Lazy::new(|| {
    NearIntCounter::new(
