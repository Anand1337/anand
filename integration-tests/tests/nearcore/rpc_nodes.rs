            ...
            Box::new(move |_ctx| {
                ...
                spawn_interruptible(async move {
                    ...
                    if block_view.header.height > 1 {
                        let client =
                            JsonRpcClient::connect(&format!("http://{}", rpc_addrs_copy[0]));
...
fn test_protocol_config_rpc() {
    ...
    cluster.exec_until_stop(|genesis, rpc_addrs, _| async move {
        ...
        let config_response = client
            .call(near_jsonrpc_primitives::types::config::RpcProtocolConfigRequest {
                block_reference: near_primitives::types::BlockReference::Finality(Finality::None),
            })
            .await
            .unwrap();
        assert_ne!(
            config_response.runtime_config.storage_amount_per_byte,
            genesis.config.runtime_config.storage_amount_per_byte
        );
        assert_eq!(config_response.runtime_config.storage_amount_per_byte, 10u128.pow(19));
        System::current().stop();
    });
}
...
fn test_query_rpc_account_view_account_doesnt_exist_must_return_error() {
    ...
    cluster.exec_until_stop(|_, rpc_addrs, _| async move {
        ...
        assert!(
            matches!(
                handler_error,
                Ok(methods::query::RpcQueryError::UnknownAccount { ref requested_account_id, .. })
                if requested_account_id.as_ref() == "accountdoesntexist.0",
            ),
            "expected an UnknownAccount handler error response, got [{:?}]",
            handler_error
        );
                        ...
                        assert!(
                            matches!(
                                ...
                                Ok(methods::broadcast_tx_commit::RpcTransactionError::InvalidTransaction {
                                    context: near_primitives::errors::InvalidTxError::NotEnoughBalance {
                                        ref signer_id,
                ...
                if let Ok(Ok(block)) = res {
                    if block.header.height > 10 {
                        ...
                        assert_eq!(
                            ...
                            request.signed_transaction.get_hash()
                        );
                        System::current().stop();
                        break;
                    }
...
fn test_send_tx_sync_to_lightclient_must_be_routed() {
    ...
    cluster.exec_until_stop(|genesis, rpc_addrs, clients| async move {
        ...
        let client = JsonRpcClient::connect(&format!("http://{}", rpc_addrs[1]));
                    ...
                    if block.header.height > 10 {
                        ...
                        assert!(
                            matches!(
                                ...
                                if transaction_hash == request.signed_transaction.get_hash()
                            ),
                    ...
                    if block.header.height > 10 {
                        let request =
                            methods::EXPERIMENTAL_tx_status::RpcTransactionStatusRequest {
                                transaction_info:
                                    methods::EXPERIMENTAL_tx_status::TransactionInfo::Transaction(
                                        transaction.clone(),
                                    ),
                            };
                        let handler_error = client
                        ...
                        assert!(
                            matches!(
                                handler_error,
                                Ok(methods::EXPERIMENTAL_broadcast_tx_sync::RpcTransactionError::UnknownTransaction {
                                    requested_transaction_hash
                                })
                                if requested_transaction_hash == transaction.get_hash()
                            ),
        ...
        spawn_interruptible(async move {
            loop {
                ...
                if let Ok(Ok(block)) = res {
                    if block.header.height > 10 {
                        let handler_error = client
                            .call(methods::EXPERIMENTAL_check_tx::RpcCheckTxRequest {
                                signed_transaction: transaction,
                            })
                            .await
                            .expect_err("request must not succeed")
                        ...
                        assert!(
                            matches!(
                                handler_error,
                                Ok(methods::EXPERIMENTAL_broadcast_tx_sync::RpcTransactionError::DoesNotTrackShard)
                            ),
                            "expected a DoesNotTrackShard handler error response, got [{:?}]",
                            handler_error
                        );
                        System::current().stop();
                        break;
