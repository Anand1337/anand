...
#[derive(Clone)]
pub struct MutableConfigValue<T>(Rc<RefCell<T>>);
...
pub struct ClientConfig {
    ...
    pub ttl_account_id_router: Duration,
    /// Horizon at which instead of fetching block, fetch full state.
    pub block_fetch_horizon: MutableConfigValue<BlockHeightDelta>,
    /// Horizon to step from the latest block when fetching state.
    pub state_fetch_horizon: NumBlocks,
    /// Time between check to perform catchup.
    pub catchup_step_period: Duration,
    ...
    pub doosmslug_step_period: Duration,
    /// Behind this horizon header fetch kicks in.
    pub block_header_fetch_horizon: MutableConfigValue<BlockHeightDelta>,
    /// Garbage collection configuration.
    pub gc: GCConfig,
    /// Accounts that this client tracks
    pub tracked_accounts: Vec<AccountId>,
...
impl ClientConfig {
    pub fn new(initial_config: StaticClientConfig) -> Self {
        Self {
            version: initial_config.version,
            chain_id: initial_config.chain_id,
            rpc_addr: initial_config.rpc_addr,
            block_production_tracking_delay: initial_config.block_production_tracking_delay,
            min_block_production_delay: initial_config.min_block_production_delay,
            max_block_production_delay: initial_config.max_block_production_delay,
            ...
            block_fetch_horizon: MutableConfigValue::new(0),
            ...
            block_header_fetch_horizon: MutableConfigValue::new(initial_config.block_fetch_horizon),
            gc: Default::default(),
...
// Config values that can be dynamically updated while the node is running
